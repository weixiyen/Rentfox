...
class Tenant_lease(object):
    ...
    def getUnitsByTenantId(tenantid):
        ...
        units = meta.Session.query(Unit.id, Unit.label, Property.name).join(Property, Lease, Tenant_lease, Tenant, Tenant_company).filter(Tenant.id==tenantid).filter(Tenant_company.companyid==request.environ.get('COMPANY_ID')).order_by(desc(Lease.startdate)).all()
        
        units_array = []
        for unit in units:
            units_array.append({'id': unit.id, 'label': '{0} #{1}'.format(unit.name, unit.label)})
        return units_array
