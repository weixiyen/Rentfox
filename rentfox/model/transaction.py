...
from sqlalchemy import schema, types, and_, or_, asc, desc, func
...
class Transaction(object):
    ...
    def get_prev_total(date):
        ...
        min = date.minute
        sec = date.second
        date = datetime.datetime(year,mo,day,hr,min,sec)
        return meta.Session.query(func.sum(Transaction.amount)).filter(and_(Transaction.date<date,\
                                                                            Transaction.deleted==None)).first()
    ...
    @staticmethod
    def get_date_period(datePeriod):
        ...
        if datePeriod == 'thisMonth':
            ...
            if m in [1,2,3]:
                y = y - 1
            startDate = [y,q[0][0],q[0][1]]
            endDate = [y,q[1][0],q[1][1]]
    ...
    def record_prev_paid(**kwargs):
        ...
        if dayDiff < 1000 and (m < curM and y == curY ) or y < curY :
            while (True):
